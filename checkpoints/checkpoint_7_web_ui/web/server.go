...
// BroadcastToolCall sends a tool call notification to all WebSocket clients.
func (s *Server) BroadcastToolCall(ctx context.Context, name, status string) {
	eventType := "tool_call"
	if status == "complete" {
		eventType = "tool_result"
	}
	s.sendToAll(ctx, wsResponse{Type: eventType, Name: name, Status: status})
}
