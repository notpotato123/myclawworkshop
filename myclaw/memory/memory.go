...
// hyphens, and underscores are allowed; everything else becomes an underscore.
...
// Save stores a memory. If the key already exists, the content and updated
// timestamp are replaced; the original created timestamp is preserved.
func (s *Store) Save(key, content string) error {
	now := time.Now().UTC().Format(time.RFC3339)
	created := now

	if existing, err := os.ReadFile(s.path(key)); err == nil {
